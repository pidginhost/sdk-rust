/*
 * Pidgin Host API
 *
 * The Pidgin Host API allows you to manage Services and resources within the Pidgin Host cloud in a simple, programmatic way using conventional HTTP requests. All of the functionality that you are familiar with in the Pidgin Host control panel is also available through the API, allowing you to script the complex actions that your situation requires.
 *
 * The version of the OpenAPI document: 1.0.0 (v1)
 * Contact: support@pidginhost.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// LbFirewallRuleActionEnum : * `ACCEPT` - Accept * `DROP` - Drop * `REJECT` - Reject
/// * `ACCEPT` - Accept * `DROP` - Drop * `REJECT` - Reject
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum LbFirewallRuleActionEnum {
    #[serde(rename = "ACCEPT")]
    Accept,
    #[serde(rename = "DROP")]
    Drop,
    #[serde(rename = "REJECT")]
    Reject,

}

impl std::fmt::Display for LbFirewallRuleActionEnum {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::Accept => write!(f, "ACCEPT"),
            Self::Drop => write!(f, "DROP"),
            Self::Reject => write!(f, "REJECT"),
        }
    }
}

impl Default for LbFirewallRuleActionEnum {
    fn default() -> LbFirewallRuleActionEnum {
        Self::Accept
    }
}

