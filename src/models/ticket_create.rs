/*
 * Pidgin Host API
 *
 * The Pidgin Host API allows you to manage Services and resources within the Pidgin Host cloud in a simple, programmatic way using conventional HTTP requests. All of the functionality that you are familiar with in the Pidgin Host control panel is also available through the API, allowing you to script the complex actions that your situation requires.
 *
 * The version of the OpenAPI document: 1.0.0 (v1)
 * Contact: support@pidginhost.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct TicketCreate {
    #[serde(rename = "subject")]
    pub subject: String,
    #[serde(rename = "department")]
    pub department: i32,
    #[serde(rename = "priority", skip_serializing_if = "Option::is_none")]
    pub priority: Option<models::TicketCreatePriorityEnum>,
    #[serde(rename = "service_id", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub service_id: Option<Option<i32>>,
    #[serde(rename = "message")]
    pub message: String,
    #[serde(rename = "attachment", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub attachment: Option<Option<String>>,
}

impl TicketCreate {
    pub fn new(subject: String, department: i32, message: String) -> TicketCreate {
        TicketCreate {
            subject,
            department,
            priority: None,
            service_id: None,
            message,
            attachment: None,
        }
    }
}

