/*
 * Pidgin Host API
 *
 * The Pidgin Host API allows you to manage Services and resources within the Pidgin Host cloud in a simple, programmatic way using conventional HTTP requests. All of the functionality that you are familiar with in the Pidgin Host control panel is also available through the API, allowing you to script the complex actions that your situation requires.
 *
 * The version of the OpenAPI document: 1.0.0 (v1)
 * Contact: support@pidginhost.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct ServerDetail {
    #[serde(rename = "id")]
    pub id: i32,
    #[serde(rename = "hostname")]
    pub hostname: String,
    #[serde(rename = "project", skip_serializing_if = "Option::is_none")]
    pub project: Option<String>,
    #[serde(rename = "image")]
    pub image: String,
    #[serde(rename = "package")]
    pub package: String,
    #[serde(rename = "cpus")]
    pub cpus: i32,
    #[serde(rename = "memory")]
    pub memory: i32,
    #[serde(rename = "disk_size")]
    pub disk_size: i32,
    #[serde(rename = "machine")]
    pub machine: std::collections::HashMap<String, serde_json::Value>,
    #[serde(rename = "volumes")]
    pub volumes: Vec<models::Volume>,
    #[serde(rename = "networks")]
    pub networks: std::collections::HashMap<String, serde_json::Value>,
    #[serde(rename = "password", skip_serializing_if = "Option::is_none")]
    pub password: Option<String>,
    #[serde(rename = "status")]
    pub status: models::StatusA57Enum,
    #[serde(rename = "username")]
    pub username: String,
    /// Prevents the server from being destroyed until disabled.
    #[serde(rename = "destroy_protection")]
    pub destroy_protection: bool,
    /// Enables Proxmox HA — automatic restart and migration on node failure.
    #[serde(rename = "ha_enabled")]
    pub ha_enabled: bool,
}

impl ServerDetail {
    pub fn new(id: i32, hostname: String, image: String, package: String, cpus: i32, memory: i32, disk_size: i32, machine: std::collections::HashMap<String, serde_json::Value>, volumes: Vec<models::Volume>, networks: std::collections::HashMap<String, serde_json::Value>, status: models::StatusA57Enum, username: String, destroy_protection: bool, ha_enabled: bool) -> ServerDetail {
        ServerDetail {
            id,
            hostname,
            project: None,
            image,
            package,
            cpus,
            memory,
            disk_size,
            machine,
            volumes,
            networks,
            password: None,
            status,
            username,
            destroy_protection,
            ha_enabled,
        }
    }
}

